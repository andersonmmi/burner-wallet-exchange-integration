...
class App extends Component {
      ...
      if (result) {
        ...
        try {
          if (
            ...
            window.web3.eth &&
            typeof window.web3.eth.getAccounts == "function" &&
            isArrayAndHasEntries(await window.web3.eth.getAccounts())
          ) {
            document.getElementById("main").style.backgroundImage =
              "linear-gradient(#553319, #ca6e28)";
    ...
    if (window.location.pathname) {
      ...
      if (window.location.pathname.indexOf("/pk") >= 0) {
        ...
        if (base64encodedPK.length == 64 || base64encodedPK.length == 66) {
          ...
          rawPK = tempweb3.utils.bytesToHex(
  ...
  async poll() {
    ...
    if (this.state.account) {
      ...
      if (
        xdaiBalance < 0.01 &&
        singleBadgeId &&
        !this.state.switchedToSingleBadge
      ) {
  ...
  async ensLookup(name) {
    ...
    const { Contract } = core.getWeb3(MAINNET_CHAIN_ID).eth;
    const ensResolver = new Contract(
